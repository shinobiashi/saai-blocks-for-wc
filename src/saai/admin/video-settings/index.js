	...
	const handleSave = () => {
		...
		apiFetch({
			...
			.then(() => {
				setNotice({
					status: 'success',
					message: __('Settings saved.', 'saai-blocks-for-wc'),
				});
			})
			.catch((error) => {
				setNotice({
					...
					message: sprintf(
						...
						error?.message ??
							__('Unknown error', 'saai-blocks-for-wc')
					),
				});
			})
		...
		<div className="saai-admin-layout">
			<div className="saai-admin__header">
				<div className="saai-admin__header-wrapper">
					<h1>
						{__('Video Gallery Settings', 'saai-blocks-for-wc')}
					</h1>
				</div>
			</div>
...
document.addEventListener('DOMContentLoaded', () => {
	const root = document.querySelector('#root-saai-blocks-for-wc-settings');
	if (root) {
