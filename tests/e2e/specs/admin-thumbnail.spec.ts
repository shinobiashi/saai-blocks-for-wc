...
test.describe('Admin panel – video thumbnail', () => {
	test('thumbnail section renders with Set thumbnail button', async ({
		page,
	}) => {
		...
		const videosTab = page.locator(
			'#woocommerce-product-data .saai_product_video_options'
		);
		...
		const thumbSection = page
			.locator('.saai-video-item__thumb-section')
			.first();
	...
	test('PHP Approach 1: FFmpeg thumbnail is resolved for existing video', async ({
		page,
	}) => {
