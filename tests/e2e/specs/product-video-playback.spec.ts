...
test.describe('Product Video – inline playback flow', () => {
	test('clicking video thumbnail in strip shows video player in gallery', async ({
		page,
	}) => {
		...
		const videoThumbSrc = await videoThumbInStrip
			.locator('img')
			.getAttribute('src');
		...
		const isVideoSlide = await activeSlide.evaluate((el) =>
			el.classList.contains('saai-video-thumb')
		);
		...
		const videoPlayer = page.locator(
			'.saai-video-thumb .saai-player--wp-media, .saai-video-thumb .saai-player-wrap'
		);
