...
test( 'video size diagnosis', async ( { page } ) => {
	...
	const sizes = await page.evaluate( () => {
		...
		const activeSlide  = document.querySelector( '.saai-video-thumb.flex-active-slide' );
		const video        = document.querySelector( '.saai-player--wp-media' ) as HTMLVideoElement | null;
		const link         = document.querySelector( '.saai-video-thumb .saai-video-thumb__link' ) as HTMLElement | null;
		const thumbStrip   = document.querySelector( '.flex-control-thumbs' );
		const blockWrapper = document.querySelector( '.wp-block-woocommerce-product-image-gallery' );
		...
		return {
			...
			video:             rect( video ),
			videoNatural:      video ? { w: video.videoWidth, h: video.videoHeight } : null,
