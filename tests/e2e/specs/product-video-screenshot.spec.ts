...
test('gallery screenshot and data-thumb check', async ({ page }) => {
	...
	const videoThumbDataThumb = await page
		.locator('.saai-video-thumb')
		.getAttribute('data-thumb');
	console.log(
		'\n=== .saai-video-thumb[data-thumb] =',
		videoThumbDataThumb,
		'==='
	);

	// Check the thumbnail strip (ol.flex-control-thumbs).
	const thumbStripHtml = await page
		.locator('.flex-control-thumbs')
		.innerHTML()
		.catch(() => 'NOT FOUND');
	console.log('\n=== .flex-control-thumbs ===\n', thumbStripHtml);
	...
	await page.screenshot({
		path: 'tests/e2e/screenshots/product-page-viewport.png',
		fullPage: false,
	});

	// Screenshot of full page.
	await page.screenshot({
		path: 'tests/e2e/screenshots/product-page-full.png',
		fullPage: true,
	});
});
