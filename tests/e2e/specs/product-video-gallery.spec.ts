...
test.describe('Product Video Gallery – ID 31 (hat)', () => {
	...
	test('gallery DOM structure', async ({ page }) => {
		...
		const galleryHtml = await page
			.locator('.woocommerce-product-gallery')
			.innerHTML()
			.catch(() => 'NOT FOUND');
		console.log(
		...
		console.log(
			'\n=== .saai-video-thumb count on page:',
			allVideoThumbs,
			'==='
		);
		...
		const scriptSrcs = await page.evaluate(() =>
			Array.from(document.querySelectorAll('script[src]'))
				...
				.filter(
					(s) => s.includes('product-video') || s.includes('saai')
				)
		);
		console.log('\n=== SAAI scripts loaded ===\n', scriptSrcs);
	});
	...
	test('clicking video thumbnail opens lightbox', async ({ page }) => {
		...
		const player = lightbox.locator(
			'.saai-lightbox__player video, .saai-lightbox__player iframe'
		);
