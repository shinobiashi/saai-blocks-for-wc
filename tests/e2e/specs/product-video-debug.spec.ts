...
test( 'JS timing and flexslider init debug', async ( { page } ) => {
	...
	const flexsliderReady = await page.evaluate( () => {
		...
		return $g.data( 'flexslider' ) ? 'FLEXSLIDER INITIALIZED' : 'NOT INITIALIZED';
	} );
	...
	const beforeCallbackAttached = await page.evaluate( () => {
		...
		return typeof fs.vars.before === 'function' ? fs.vars.before.toString().slice( 0, 100 ) : false;
	} );
	...
	console.log( '\n=== wc-product-gallery-after-init fired ===', afterInitFired );
	...
	const afterClickState = await page.evaluate( () => {
		...
		const player = document.querySelector( '.saai-player, .saai-player-wrap' );
		return {
