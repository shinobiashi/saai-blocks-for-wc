...
test( 'get wc params and flexslider state', async ( { page } ) => {
    ...
    const info = await page.evaluate( () => {
        ...
        const fs = gallery ? (window as any).jQuery(gallery).data('flexslider') : null;
        return {
    ...
    await page.evaluate( () => {
        ...
        fs.vars.before = function(slider: any) {
            ...
            console.log(`[SAAI DEBUG] before() - viewport=${vpBefore}, nextSlide.outerHeight=${nextH}, animatingTo=${slider.animatingTo}`);
            origBefore(slider);
            const vpAfterOrig = $(gallery).find('.flex-viewport').height();
            console.log(`[SAAI DEBUG] after origBefore - viewport=${vpAfterOrig}`);
        };
    });
    ...
    const vpH = await page.evaluate(() => {
        ...
        return { h: (vp as HTMLElement)?.offsetHeight, inline: (vp as HTMLElement)?.style.height };
    });
