...
- [x] `woocommerce_product_thumbnails` フックで動画サムネイルを指定位置に挿入
- [x] `gallery_order` に基づいて動画間の表示順を制御（商品画像との混在順序制御は Phase 1.5 で対応予定）
- [x] フロントエンド JS:
...
- [x] YouTube / Vimeo のサムネイル自動取得（YouTube: mqdefault.jpg）
- [x] WordPress メディア動画の `<video>` タグレンダリング
- [x] `wp_enqueue_scripts` でフロントスクリプト・スタイル登録（商品ページのみ）
